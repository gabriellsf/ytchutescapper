    ...
    while type_index < len(type):
        while category_index < len(category):
            ...
            while True:
                ...
                file_manager.write_data(request_extend_video.text,"video/" + category[category_index] + "/list/" + date.today().strftime("%Y%m%d")  + "/list_" + type[type_index], "extend_"+ str(offset) + ".txt")
